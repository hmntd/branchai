...
use crate::services::config_service::GitAccount;
...
pub fn apply_repo_account(repo_path: String, account: GitAccount) -> Result<(), String> {
    ...
    // 1. Set local user.name and user.email in repository .git/config
    if !account.username.is_empty() {
        config.set_str("user.name", &account.username).map_err(|e| e.message().to_string())?;
    }
    if !account.email.is_empty() {
        config.set_str("user.email", &account.email).map_err(|e| e.message().to_string())?;
    }

    // 2. Set core.sshCommand if SSH authentication with specific key
    if account.auth_type == "ssh" {
        if let Some(ref key_path) = account.ssh_key_path {
            if !key_path.trim().is_empty() {
                let clean_key_path = key_path.replace("~", &std::env::var("HOME").unwrap_or_default());
                let ssh_cmd = format!("ssh -i \"{}\" -o IdentitiesOnly=yes", clean_key_path);
                config.set_str("core.sshCommand", &ssh_cmd).map_err(|e| e.message().to_string())?;
            }
        }
    } else {
        // Remove core.sshCommand if switching to HTTPS/PAT
...
pub async fn test_account_connection(account: GitAccount) -> Result<String, String> {
    ...
    if account.auth_type == "ssh" {
        ...
        let clean_path = key_path.replace("~", &std::env::var("HOME").unwrap_or_default());
