...
<template>
  <template v-for="node in nodes" :key="node.fullPath">
    ...
    <div
      ...
      :style="indent(depth)"
      @click="emit('toggleFolder', node.fullPath)"
    >
    ...
    <TreeNodes
      v-if="node.isDir && isOpen(node.fullPath)"
      :nodes="node.children"
      :staged="staged"
      :read-only="readOnly"
      :depth="depth + 1"
      ...
      @inspect="(f) => emit('inspect', f)"
      @stage="(f) => emit('stage', f)"
      @unstage="(f) => emit('unstage', f)"
    ...
    <div
      ...
      class="file-row tree-file-row"
      :class="{ staged }"
      :style="indent(depth)"
      @click="emit('inspect', node.file)"
      @contextmenu.prevent="emit('contextMenu', $event, node.file)"
    >
      ...
      <button
        v-if="!readOnly && !staged"
        class="stage-btn"
        @click.stop="emit('stage', node.file)"
        title="Stage File"
      >
        <Plus :size="11" />
      </button>
      <button
        v-else-if="!readOnly && staged"
        class="stage-btn"
        @click.stop="emit('unstage', node.file)"
        title="Unstage File"
      >
        <Minus :size="11" />
      </button>
...
.tree-file-row {
  /* inherits .file-row from parent scope, just adjusts text */
}
...
/* Shared with parent — imported via non-scoped globals */
.file-row {
  display: flex;
  align-items: center;
  gap: 6px;
